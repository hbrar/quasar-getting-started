...
2. To verify Babel is working.
	 ...
	 a. Install @babel/cli as dev dependencies using `yarn add --dev @babel/cli` and later remove since Webpack will be taking care of it.
		
      Expect: You should see @babel/cli under node_modules and @babel/cli gets added as dev dependency in myapp/package.json
...
-	Create src/index.html file with your template:
...
- Update webpack config as follows:
		        
		const HtmlWebpackPlugin = require('html-webpack-plugin');
		        
- Update plugins section of module.exports with below code.
...
- Validate if new template is in place.
